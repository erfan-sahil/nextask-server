...
export const findPopulatedTaskCommentOrThrow = async (commentId, taskId) => {
  const comment = await populateTaskComment(
    TaskComment.findOne({ _id: commentId, taskId })
  );
...
export const ensureActorCanCreateTaskComment = async (
  workspace,
  projectId,
  userId
) => {
...
const canManageAnyComment = (membership) =>
  [WORKSPACE_MEMBER_ROLE.OWNER, WORKSPACE_MEMBER_ROLE.ADMIN].includes(
    membership.role
  );

export const ensureActorCanUpdateTaskComment = async (
  workspace,
  projectId,
  comment,
  userId
  ...
  if (
    ...
    !canManageAnyComment(membership)
  ) {
...
export const ensureActorCanDeleteTaskComment = async (
  workspace,
  projectId,
  comment,
  userId
