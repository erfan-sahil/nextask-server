...
const populateNotification = (query) => query.populate('actorId', USER_POPULATE_FIELDS);

const emitNotification = (notification) => {
  getSocketServer()
    ?.to(userRoom(notification.recipientId.toString()))
...
export const notificationService = {
  ...
  async mentionedUserIds(workspaceId, content) {
    const usernames = [...content.matchAll(/@([a-z0-9_-]+)/gi)].map((match) =>
      match[1].toLowerCase()
    );
